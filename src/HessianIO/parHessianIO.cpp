...
#include "parHessianIO.h"

int CParHessianOp::m_nValueSize = 0;
int CParHessianOp::m_nPageSize = 0;
int CParHessianOp::m_nBlockSize = 0;
long long CParHessianOp::m_nOffset = 0;

int CParHessianOp::m_nPageCapacity = 0;	//the number of values that a page can store
int CParHessianOp::m_nPagesForARow = 0;	//the number of pages to store a row

int CParHessianOp::m_nNumofThread = 0;		//the number of threads to read a row
FILE **CParHessianOp::m_pFileReadIn = NULL;
ThreadParameter *CParHessianOp::m_pThreadArg = NULL;
bool CParHessianOp::m_isFirst = false;
float_point* CParHessianOp::pfHessianFullRow = NULL;
 ...
 */
bool CParHessianOp::WriteHessianRows(FILE *&writeOut, float_point *pfHessianRows, SubMatrix &subMatrix)
{
 ...
 */
bool CParHessianOp::ReadHessianRow(FILE *&readIn, const int &nHessianIndexofRow, float_point *pfHessianRow)
{
	...
	int nSSDRowIndex = nHessianIndexofRow - m_nNumofCachedHessianRow;
...
void *CParHessianOp::ReadRow(void *pThreadParameter)
{
	...
	for(int i = nTid; i < m_nPagesForARow; i+=m_nNumofThread)
	{
		...
		if(nNumofRemainingValue >= m_nPageSize)
		{
			m_fileOps->ReadPartOfRowFromFile(m_pFileReadIn[nTid], pfStartPos, m_nPageSize, nRowPos);
			if(nNumofRemainingValue == m_nPageSize)
				break;
		}
		else if(nNumofRemainingValue > 0)
		{
			int nNumofValueToRead = nNumofRemainingValue;
			m_fileOps->ReadPartOfRowFromFile(m_pFileReadIn[nTid], pfStartPos, nNumofValueToRead, nRowPos);
			break;
		}
		else
		{
