 ...
 */
bool CSeqHessianOp::WriteHessianRows(FILE *&writeOut, float_point *pfHessianRows, SubMatrix &subMatrix)
{
 ...
 */
bool CSeqHessianOp::ReadHessianRow(FILE *&readIn, const int &nIndexofRow, float_point *pfHessianRow)
{
...
//	clock_gettime(CLOCK_REALTIME, &time1);
	...
	if(m_nRowStartPos1 != -1)
	{
		...
		m_fileOps->ReadPartOfRowFromFile(readIn, pfHessianRow, nSizeofFirstPart, nIndexofFirstElement);
	}
	...
	if(m_nRowStartPos2 != -1)
	{
		...
		//cout << " index of row " << nIndexofRow << endl;
		m_fileOps->ReadPartOfRowFromFile(readIn, pfHessianRow + nSizeofFirstPart, nSizeofSecondPart, nIndexofFirstElement);
	}
