...
void trainingByGPU(vector<vector<float_point> > &v_v_DocVector, data_info &SDataInfo, SVMParam &param)
{
	...
	CHessianIOOps::m_nNumofDimensions = v_v_DocVector.front().size();
	...
	long nMemForSamples = (ops.m_nNumofDimensions * ops.m_nTotalNumofInstance * 2);
...
*/	cout << nNumofHessianRow << " rows cached in RAM" << endl;
	...
	checkCudaErrors(cudaMallocHost((void**)&CHessianIOOps::m_pfHessianRowsInHostMem,
	...
	CHessianIOOps::m_pfHessianDiag = new float_point[ops.m_nTotalNumofInstance];
//	ops.m_pfHessianDiagTest = new float_point[ops.m_nTotalNumofInstance];
	...
	gettimeofday(&t1, NULL);
	bool bWriteHessian = ops.WriteHessian(strHessianMatrixFileName, strDiagHessianFileName, v_v_DocVector);
	...
	cout << " rho = " << model.rho[0] << "; # of SVs = " << model.nNumofSV << endl;
